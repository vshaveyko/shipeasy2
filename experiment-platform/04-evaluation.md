...
export function evalGate(gate: Gate, user: User): boolean {
  ...
  for (const rule of gate.rules) if (!matchRule(rule, user)) return false;
...
const { inExperiment, params } = flags.getExperiment(
  ...
  (raw) => {
    // decoder — validates server shape
...
const RUNNERS = {
  ...
  java: (input) =>
    `java -cp packages/sdk_java/build/libs/sdk.jar com.flaglab.HashTest ${JSON.stringify(input)}`,
};
...
for (const [lang, runner] of Object.entries(RUNNERS)) {
  for (const { input, expected } of VECTORS) {
    try {
      ...
      if (result !== expected) {
        console.error(
          `FAIL [${lang}] input="${input}": got 0x${result.toString(16).toUpperCase().padStart(8, "0")}, expected 0x${expected.toString(16).toUpperCase().padStart(8, "0")}`,
        );
        failures++;
      }
    } catch (err) {
