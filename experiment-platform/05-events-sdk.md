...
export class EventBuffer {
  ...
  pushExposure(experiment: string, group: string, userId: string, anonId: string) {
    ...
    this.enqueue({ type: 'exposure', experiment, group, user_id: userId, anonymous_id: anonId, ts: Date.now() })
  }
  ...
  pushMetric(eventName: string, value: number | undefined, userId: string, anonId: string) {
    this.enqueue({ type: 'metric', event_name: eventName, value, user_id: userId, anonymous_id: anonId, ts: Date.now() })
  }
  ...
  async alias(anonymousId: string, userId: string): Promise<void> {
    ...
    try { localStorage.setItem('__flaglab_pending_alias', JSON.stringify(record)) } catch {}
    ...
    try { localStorage.removeItem('__flaglab_pending_alias') } catch {}
  }
