...
function computeAggregate(
  ...
  if (agg === 'avg')          return postExposure.length > 0
    ? postExposure.reduce((s, e) => s + e.value, 0) / postExposure.length : 0
...
export async function runAnalysisCron(env: Env): Promise<void> {
  ...
  await env.ANALYSIS_QUEUE.sendBatch(
    runningProjects.map((p) => ({ body: { project_id: p.id } }))
  )
  ...
  await db.insert(systemHealth)
    .values({ key: 'analysis_cron', lastFiredAt: new Date().toISOString(), projectsEnqueued: runningProjects.length })
    .onConflictDoUpdate({ target: [systemHealth.key], set: { lastFiredAt: new Date().toISOString(), projectsEnqueued: runningProjects.length } })
...
export async function consumeAnalysis(batch: MessageBatch<AnalysisMessage>, env: Env) {
  ...
  try {
    ...
    console.error(JSON.stringify({ event: 'analysis_failed', project_id: msg.project_id, trigger: msg.trigger, error: String(err) }))
...
async function analyzeProject(msg: AnalysisMessage, env: Env): Promise<void> {
  ...
  if (msg.trigger === 'experiment_stopped' && msg.experiment) {
    ...
    expsToAnalyze = await db.select().from(experiments)
      .where(and(
  ...
  for (const exp of expsToAnalyze) {
    const isFinal = msg.trigger === 'experiment_stopped'
      || (msg.trigger === 'reanalyze' && exp.status === 'stopped')
    try {
      await analyzeExperiment(exp, msg.project_id, env, isFinal)
    } catch (err) {
      console.error(JSON.stringify({
        event: 'experiment_analysis_failed', project_id: msg.project_id,
        experiment: exp.name, trigger: msg.trigger, error: String(err),
      }))
    }
  }

  await db.update(analysisFailures)
...
const result = await db.update(experiments)
  ...
  .where(and(eq(experiments.id, id), eq(experiments.projectId, projectId), eq(experiments.status, 'running')))
...
if (excludedCount > 0) {
  console.warn(JSON.stringify({
    event: 'alias_collision_exclusion', experiment: experimentName,
  ...
  for (const exposure of canonicalExposures) {
    ...
    if (!agg) {
      ...
      switch (metric.aggregation) {
        case 'count_users':  userValue = 1;  break
        case 'count_events': userValue = agg.event_count;  break
        case 'sum':          userValue = agg.total_value;  break
        case 'avg':          userValue = agg.event_count > 0 ? agg.total_value / agg.event_count : 0;  break
        default:             userValue = 0   // retention_Nd: handled by needsR2 path above
      }
    }
...
if (srm_detected) {
  console.error(JSON.stringify({
    event: 'srm_detected', experiment: exp.name, project_id: projectId,
    actual: actualGroupCounts, expected: exp.groups.map(g => ({ name: g.name, weight: g.weight })),
    p_value: srm_p_value,
  }))
...
async function startExperiment(projectId: string, experimentName: string, env: Env, plan: Plan) {
  ...
  const existingExp = await db.select({ status: experiments.status, startedAt: experiments.startedAt, stoppedAt: experiments.stoppedAt })
    .from(experiments)
    .where(and(eq(experiments.projectId, projectId), eq(experiments.name, experimentName)))
    .get()

  if (cupedEligible && existingExp?.status === 'stopped' && existingExp.startedAt && existingExp.stoppedAt) {
    ...
    if (priorStartMs < newStartMs && priorStopMs > baselineStart) {
      ...
      console.warn(JSON.stringify({
        ...
        project_id: projectId, experiment: experimentName,
        reason: 'Restart baseline window overlaps prior run period — CUPED would attenuate effect',
        prior_started_at: existingExp.startedAt, prior_stopped_at: existingExp.updatedAt,
        new_baseline_start: new Date(baselineStart).toISOString(), new_started_at: startedAt,
      }))
    }
  }
...
async function freezeCupedBaselines(env: Env, projectId: string, experimentName: string, startedAt: string): Promise<void> {
  ...
  for (const metric of attachedMetrics) {
    ...
    if (rows.length === 0) {
      console.warn(JSON.stringify({ event: 'cuped_no_baseline_data', projectId, metric: metric.name }))
      continue
    }
    ...
    const cap        = sortedVals[Math.floor(sortedVals.length * 0.99) - 1]
                       ?? sortedVals[sortedVals.length - 1]
    const clipped    = rows.filter((r: any) => r.avg_value > cap).length
    if (clipped > 0) {
      console.log(JSON.stringify({
        event: 'cuped_baseline_winsorized', projectId, metric: metric.name,
        cap, clipped_users: clipped, total_users: rows.length,
    ...
    for (let i = 0; i < winsorizedRows.length; i += 100) {
      await db.batch(winsorizedRows.slice(i, i + 100).map((r: any) =>
        db.insert(userMetricBaseline).values({
          projectId, userId: r.user_id, metricName: metric.name,
          avgValue: r.avg_value, updatedDs: startedAt,
        }).onConflictDoUpdate({ target: [userMetricBaseline.projectId, userMetricBaseline.userId, userMetricBaseline.metricName], set: { avgValue: r.avg_value, updatedDs: startedAt } })
      ))
    }
  }
...
```typescript
function applyCuped(values: Map<string, number>, baselines: Map<string, number>): { result: Map<string, number>; overlap_pct: number } {
  ...
  if (users.length < 100 || overlap_pct < 0.5) {
    console.warn(JSON.stringify({ event: 'cuped_low_overlap', overlap_pct, users_with_baseline: users.length, total_users: values.size }))
...
```typescript
function msprtTau(metric: { min_detectable_effect: number | null }, controlValues: number[]): number {
...
export function computeMSPRT(
  ...
  const tau2   = tau * tau
  const lambda = Math.sqrt(V / (V + tau2)) * Math.exp(tau2 * delta * delta / (2 * V * (V + tau2)))
...
let msprtResult: { msprt_lambda: number | null; msprt_significant: number | null } =
  { msprt_lambda: null, msprt_significant: null }
...
await db.insert(experimentResults).values({
  projectId, experiment: expName, metric: metricName, groupName, ds,
  n:              result.n_test,
  ...
  target: [experimentResults.projectId, experimentResults.experiment, experimentResults.metric, experimentResults.groupName, experimentResults.ds],
  set: {
    n: result.n_test, mean: result.mean_test, /* ... all fields ... */
    srmDetected: srmResult.srm_detected ? 1 : 0,
    msprtLambda: msprtResult.msprt_lambda,
    msprtSignificant: msprtResult.msprt_significant,
...
export function computeVerdict(
  ...
  experiment: { min_runtime_days: number; min_sample_size: number; significance_threshold: number; started_at: string; sequential_testing: boolean },
  analysisDate: string,
  srmDetected: boolean,
): VerdictOutput {

  // Guard 1: SRM invalidates the experiment
  if (srmDetected) return {
    verdict:   'invalid',
    title:     'Experiment invalid — Sample Ratio Mismatch detected',
    narrative: 'The actual ratio of users per group differs significantly from the expected ratio. ' +
  ...
  const AUTO_MDE: Record<string, number> = {
    ...
    '__auto_network_error': 0.010,  // 1pp absolute
  ...
  if (peekWarning && !experiment.sequential_testing) return {
    verdict:      'wait',
    title:        'Too early to conclude — minimum runtime not reached',
    narrative:    `Experiment has run ${daysSinceStart}d of required ${experiment.min_runtime_days}d. ` +
  ...
  if (minN < experiment.min_sample_size) return {
    verdict:      'wait',
  ...
  const perfClusterBroken = guard.filter(m =>
  ...
  if (guardrailsBroken.length > 0) return {
    verdict:      'hold',
  ...
  const goalDesc = goal.map(m => `${m.metric} +${m.delta_pct.toFixed(1)}% (p=${m.p_value.toFixed(3)})`).join('; ')
  return {
    verdict:      'ship',
    title:        'Ship it',
...
export async function runArchivalCron(env: Env): Promise<void> {
  ...
  for (const proj of allProjs) {
    try {
      ...
      console.error(JSON.stringify({ event: 'archive_failed', project_id: proj.id, ds, error: String(err) }))
    }
  }

  await getDb(env.DB).insert(systemHealth)
    .values({ key: 'archival_cron', lastFiredAt: new Date().toISOString(), projectsEnqueued: allProjs.length })
    .onConflictDoUpdate({ target: [systemHealth.key], set: { lastFiredAt: new Date().toISOString(), projectsEnqueued: allProjs.length } })
  ...
  if (archiveFailedProjects.length > 0 && env.ALERT_EMAIL) {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        text:    `Projects that failed R2 archival for ${ds}:\n\n${archiveFailedProjects.join('\n')}\n\n` +
...
async function archiveProjectDay(
  ...
  console.log(JSON.stringify({
    event: 'archive_day_complete', project_id: projectId, ds,
    exposures: exposures.length, metrics: metrics.length,
  }))
}
```
...
const aeExposureRows  = await queryAE(exposureSqlBounded(projectId, experiment.name,
                          Math.max(expStartMs, aeWindowStart), analysisFence), env)
...
if (needsR2) {
  ...
  console.log(JSON.stringify({
    event: 'r2_archive_read', project_id: projectId, experiment: experiment.name,
    r2_exposure_rows: r2ExposureRows.length,
    r2_metric_events: Object.fromEntries([...r2MetricRowsByMetric.entries()].map(([k,v]) => [k, v.length])),
  }))
}
...
export async function runRetentionPurge(env: Env): Promise<void> {
  ...
  for (const proj of allProjs) {
    ...
    const rowsToArchive = await db.select()
    ...
    if (rowsToArchive.length > 0) {
      ...
      for (const [key, rows] of grouped) {
        try {
          ...
          archiveErrors++
          console.error(JSON.stringify({ event: 'result_archive_failed', project_id: proj.id, key, error: String(err) }))
        }
      }
      ...
      if (archiveErrors === 0) {
        ...
        console.log(JSON.stringify({ event: 'result_archive_complete', project_id: proj.id, rows: rowsToArchive.length }))
      }
    }

    // Delete old daily result rows — preserve is_final=1 (terminal experiment records, handled above)
    const deleted = await db.delete(experimentResults)
      .where(and(
        eq(experimentResults.projectId, proj.id),
    ...
    if ((deleted.rowsAffected ?? 0) > 0) {
      console.log(JSON.stringify({
        event: 'retention_purge', project_id: proj.id, cutoff,
  ...
  // Purge audit log entries older than plan retention
    const auditCutoff = new Date(Date.now() - plan.audit_log_retention_days * 86_400_000)
      .toISOString()
  ...
  await db.delete(cliAuthSessions)
    .where(and(
  ...
  const unresolved = await db
    .select({ projectId: analysisFailures.projectId, failedAt: analysisFailures.failedAt, messageBody: analysisFailures.messageBody })
    .from(analysisFailures)
    .where(and(
      isNull(analysisFailures.resolvedAt),
      gt(analysisFailures.failedAt, failureCutoff),
    ))

  if (unresolved.length > 0 && env.ALERT_EMAIL) {
    const body = unresolved.map(f =>
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        text:    `The following projects failed analysis in the last 26 hours:\n\n${body}\n\n` +
