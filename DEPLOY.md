...
```
pnpm install --frozen-lockfile && pnpm --filter @shipeasy/devtools build && pnpm --filter @shipeasy/worker exec wrangler d1 migrations apply shipeasy-db --remote && pnpm --filter @shipeasy/ui exec opennextjs-cloudflare build
```
