# Shipeasy — General Install (run first)

Installs the SDK, authenticates the machine, binds the repo to a project,
provisions keys, wires the SDK into the root layout, installs the MCP
server + plugin + shared skills, and verifies the result. After this you
pick the per-feature guides you actually want (experiments, configs,
translations, bugs).
...
1. Looks up `(owner_email, domain)` against your account.
...
(e.g. recovering after someone deleted `.shipeasy`):
...
- `.shipeasy` is in `.gitignore` from a stale install → remove it from
  `.gitignore` and commit `.shipeasy`. The legacy "auto-gitignored cache"
  behaviour was removed when binding became mandatory.
...
`?se` / `?se_devtools` is in the URL (the same overlay used by the
`feedback` module to file bug reports). Discard the return value and the
client renders untranslated keys until the loader hydrates, devtools never
appears in production, and bugs can't be filed from the UI.
...
you have a reason to deviate — see [`translations.md`](./translations.md).
...
appends the i18n loader and the devtools loader.
...
## 6. Register the MCP server, plugin, and skills
...
config — both visible in the output. Re-run with `--force` only after a
human confirms.

### Install the shared skills + slash commands
...
This installs the **base** skill — `shipeasy-setup` — plus the four
feature-scoped skills. The per-feature guides each call out which skill they
rely on, but `shipeasy plugin install` is run **once** here in the general
guide. Per-feature guides only enable the corresponding **module** (via
`shipeasy modules enable <name>`); they do not re-run the plugin install.

| Skill                  | Used by guide                                                      |
| ---------------------- | ------------------------------------------------------------------ |
| `shipeasy-setup`       | This guide (general onboarding).                                   |
| `shipeasy-experiments` | [`experiments-metrics.md`](./experiments-metrics.md)               |
| `shipeasy-flags`       | [`configs-gates-killswitches.md`](./configs-gates-killswitches.md) |
...
`shipeasy mcp install --scope project`.
...
shipeasy modules list                   # all modules off by default; you'll enable per-feature later
...
## 8. Pick the features you want

By default every per-feature module is **off**. Turn on what you need with
`shipeasy modules enable <name>`, then follow the matching guide:

| Want…                                          | Run                                                                | Then read                                                          |
| ---------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------ |
| A/B tests + custom event metrics               | `shipeasy modules enable experiments`                              | [`experiments-metrics.md`](./experiments-metrics.md)               |
| Feature gates / dynamic configs / killswitches | `shipeasy modules enable gates && shipeasy modules enable configs` | [`configs-gates-killswitches.md`](./configs-gates-killswitches.md) |
| Translatable copy via CDN-served labels        | `shipeasy modules enable translations`                             | [`translations.md`](./translations.md)                             |
| In-app bug reports + feature requests          | `shipeasy modules enable feedback`                                 | [`bugs-feature-requests.md`](./bugs-feature-requests.md)           |

You can enable any subset, any time. Modules are independent.

---

## 9. Hand-off report to the user

End the session with a short report (one screen). Template:

```
✅ Shipeasy installed (general)
...
Modules:   (none enabled yet — pick from install/README.md)

Next:
  • Review `git status` / `git diff --stat` and commit (see step 10)
  • Enable the modules you need: `shipeasy modules enable <name>`
  • Follow the matching per-feature guide under install/
...
| Show enabled modules               | `shipeasy modules list`                      |
| Enable / disable a module          | `shipeasy modules {enable,disable} <name>`   |

---

## 10. Ask the user to commit the changes
...
Expected modified / untracked entries from the **general** install (skip any
rows that don't apply):

| Path                                                           | What it is                         | Commit?                                                                                                                            |
| -------------------------------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `package.json` + lockfile                                      | new SDK + React deps               | yes                                                                                                                                |
| `app/layout.tsx` (or `src/app/layout.tsx`)                     | server SDK init                    | yes                                                                                                                                |
...
(Per-feature guides each contribute their own diff footprint; commit those
alongside or as separate commits per the guide's recommendation.)
...
- **Never run `git commit` or `git push` yourself** unless the user has
  explicitly authorised it for this session.
...
git commit -m "chore: onboard Shipeasy (SDK + MCP + plugin)"
```
