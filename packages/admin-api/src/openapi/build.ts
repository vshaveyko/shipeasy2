...
function buildOperation(ctx: BuildOperationCtx): JsonObject {
    ...
    requestBody = {
      ...
      content: {
        "application/json": {
          ...
          ...(ep.examples?.request !== undefined ? { example: ep.examples.request } : {}),
        },
      },
    };
  }
    ...
    [status]: {
      ...
      content: {
        "application/json": {
          ...
          ...(ep.examples?.response !== undefined ? { example: ep.examples.response } : {}),
        },
      },
    },
  ...
  const description = [ep.description, ep.useCase ? `**Use case:** ${ep.useCase}` : null]
    .filter(Boolean)
    .join("\n\n");
