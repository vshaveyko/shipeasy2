...
test.describe("i18n Keys table — inline editing", () => {
  ...
  test.afterAll(async ({ browser }) => {
    ...
    await p.goto("/dashboard/e2e-project-id/i18n/keys");
...
test.describe("i18n Keys table — draft workflow", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    const ctx = await browser.newContext({ storageState: AUTH_FILE });
    const p = await ctx.newPage();
    await p.goto("/dashboard/e2e-project-id/i18n/drafts/new");
    ...
    await expect(p).toHaveURL(/\/dashboard\/e2e-project-id\/i18n\/drafts$/);
    await ctx.close();
  });
  ...
  test.afterAll(async ({ browser }) => {
    const ctx = await browser.newContext({ storageState: AUTH_FILE });
    const p = await ctx.newPage();
    await p.goto("/dashboard/e2e-project-id/i18n/drafts");
