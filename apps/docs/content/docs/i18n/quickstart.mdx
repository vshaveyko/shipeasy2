---
title: Quickstart
description: Onboard a project to Polylang in 10 minutes — scan, codemod, translate, publish.
---
...
<Quickstart title="The 10-minute path" time="install · scan · translate · publish">
  ...
  <QuickstartStep
    ...
    cmd="shipeasy i18n publish --profile fr:prod"
  />
</Quickstart>
...
<Steps>
  ...
  <Step title="Push keys">
    ...
    Pushes every discovered key to the dashboard. Verify in **Polylang → Label keys**.
  ...
  <Step title="Review">
    Open **Polylang → Drafts → fr:prod**. You see every key, the source value, and the proposed translation, side by side. Edit any value inline. When you're satisfied, click **Merge draft**.
...
<TileGrid>
  <Tile
    ...
    meta={
      <>
        <span>locale × env</span>
        <span>·</span>
        <span>versioned</span>
      </>
    }
  >
    Each <code>locale:env</code> pair is its own publishable manifest. Roll back any time.
  </Tile>
  <Tile
    href="/i18n/keys"
    icon="✓"
...
<DocNav
  prev={{ href: "/i18n", title: "Polylang overview" }}
  next={{ href: "/i18n/profiles", title: "Profiles" }}
/>

<DocFeedback editHref="https://github.com/shipeasy-ai/shipeasy2/edit/main/apps/docs/content/docs/i18n/quickstart.mdx" />
