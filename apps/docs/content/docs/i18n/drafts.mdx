...
This separation is the single most important guardrail in Polylang. AI translations are good but not perfect; merging without review is how you ship "Confirmer" as a button label that overflows on mobile in three locales for two weeks before anyone notices.
...
The most common path. The CLI calls Anthropic Claude (using your project's API key — see [Translations](/i18n/translations) for setup) and creates a draft with proposed values for every key missing from the target profile.
...
shipeasy i18n translate --profile fr:prod --import ./fr.xlf

# CSV with key,value columns
shipeasy i18n translate --profile fr:prod --import ./fr.csv
```
...
In the dashboard, **Polylang → Drafts → New** opens an empty draft for the chosen profile. You can paste keys and values, or use the inline editor. Useful for quick fixes ("change the FR CTA from 'Régler' to 'Payer'") that don't merit a full AI run.

## Reviewing a draft

Open **Polylang → Drafts → fr:prod**. You see a four-column view:

| Source (en:prod)  | Current (fr:prod)  | Proposed (draft)    | Decision               |
| ----------------- | ------------------ | ------------------- | ---------------------- |
...
<TileGrid>
  ...
  >
    ...
    <code>:prod</code> environments — that needs admin.
  </Tile>
  <Tile
    icon="✦"
    title="Admin"
  ...
  >
    All reviewer powers, plus publish to any environment, create/delete profiles, manage roles,
...
You can also auto-publish on merge in **Project → Polylang → Settings → Auto-publish on merge** if you trust the review step entirely. Most teams turn this on for `:dev` and `:staging` but leave `:prod` manual.
...
A typical scenario: a translator is hand-editing `fr:prod` while a nightly cron runs `--ai` on new keys. The cron's draft has a row for `checkout.cta_label`; the translator's draft also touched it. On merge of one, the other gets flagged.
...
jobs:
  translate:
    ...
    steps:
      ...
      - run: shipeasy i18n translate --profile fr:prod --ai --ci
...
<ConvertCTA
  ...
  primaryHref="/i18n/translations"
  primaryLabel="AI translations"
  secondaryHref="/i18n/profiles"
  secondaryLabel="Back to profiles"
  cmds={[
    { label: "AI draft", cmd: "shipeasy i18n translate --profile fr:prod --ai" },
    { label: "Publish merged", cmd: "shipeasy i18n publish --profile fr:prod" },
  ]}
  note="Side-by-side review · RBAC · back-translation"
/>

<DocNav
  prev={{ href: "/i18n/keys", title: "Label keys" }}
  next={{ href: "/i18n/translations", title: "AI translations" }}
/>

<DocFeedback editHref="https://github.com/shipeasy-ai/shipeasy2/edit/main/apps/docs/content/docs/i18n/drafts.mdx" />
