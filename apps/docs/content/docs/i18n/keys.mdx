---
title: Label keys
description: How Polylang names, hashes, chunks, describes, and deletes the strings your code references.
---
...
Polylang's job is to make keys feel invisible: you write English in code, the codemod hashes it, the AI translates it, and the runtime resolves it back. But understanding what a key actually is — and how it survives renames, refactors, and re-scans — is what stops your localization from drifting six months in.
...
- **Don't embed the environment**: keys are scoped to a profile, not duplicated per env.
...
Polylang detects when a source value changes (the hash changes) and marks every target translation as **stale**. Stale keys still serve their old value at the loader — nothing breaks — but they show up in the dashboard with a yellow flag and are eligible for re-translation.

```bash
shipeasy i18n translate --profile fr:prod --ai --stale-only
```
...
<ConvertCTA
  ...
  primaryHref="/i18n/drafts"
  primaryLabel="How drafts work"
  secondaryHref="/i18n/translations"
  secondaryLabel="AI translations"
  cmds={[
    { label: "Validate keys", cmd: "shipeasy i18n validate --profile en:prod ./src" },
...
<DocNav
  prev={{ href: "/i18n/profiles", title: "Profiles" }}
  next={{ href: "/i18n/drafts", title: "Drafts" }}
/>

<DocFeedback editHref="https://github.com/shipeasy-ai/shipeasy2/edit/main/apps/docs/content/docs/i18n/keys.mdx" />
