...
description: Typed config values — strings, numbers, booleans, JSON — that change instantly without a redeploy. Schema-validated, versioned, per-environment.
---
...
Like gates, every value holds an independent record per environment:

| Key             | dev                           | staging                           | prod                      |
| --------------- | ----------------------------- | --------------------------------- | ------------------------- |
| `rate-limit`    | 1000                          | 200                               | 100                       |
| `default-theme` | `dark`                        | `light`                           | `light`                   |
| `api-base-url`  | `https://api-dev.example.com` | `https://api-staging.example.com` | `https://api.example.com` |

The SDK pins to one environment at boot — see [Environments](/configs/environments).

## When to use a value vs. a gate

| Question                           | Use                                                        |
| ---------------------------------- | ---------------------------------------------------------- |
| Should I show feature X?           | [Gate](/configs/gates).                                    |
...
| Need a kill-switch?                | Gate (or a value with a `null`-equivalent fallback).       |
...
- name: Validate dynamic values
  run: shipeasy values validate ./src --env prod
```
...
<ConvertCTA
  ...
  secondaryHref="/configs/environments"
  secondaryLabel="Environments"
  cmds={[
